using ..PrevisibilityCrawler: pty, plt
using DataFrames
...
function plotprevisibility(series, signal)
    all = pty.scatter(;y=series)
    beginings = findall(x -> x == 1, diff(signal))
    signal[1] == true && pushfirst!(beginings ,1)
    ends = findall(x -> x == -1, diff(signal))
    signal[end] == true && push!(ends, length(signal))
    coverage = count(signal)/length(signal)
    shapes = pty.rect(beginings,ends,0,1;xref="x",yref="paper",line_width=0,fillcolor="#d3d3d3", opacity=.3)
    pty.plot([all], pty.Layout(;shapes=shapes, title="Coverage : $coverage", margin=attr(l=20, r=100, b=40)))
end